"""
实验三（高级Hybrid Precision算法）结果合并与分析脚本
用于提取和合并5个批次的实验三结果，生成综合性能报告
"""
...
class Experiment3ResultsAnalyzer:
    ...
    def load_batch_results(self) -> bool:
        ...
        Returns:
            是否成功加载所有文件
        """
        print("📊 开始加载实验三（高级Hybrid Precision算法）批次结果...")

        for batch_id in range(1, 6):
            result_file = self.batch_results_dir / f"hybrid_precision_batch_{batch_id}_200_samples_results.json"
            ...
            try:
                ...
                hybrid_precision_summary = summary.get("hybrid_precision", {})

                self.batch_summaries.append({
                    "batch_id": batch_id,
                    "total_samples": summary.get("total_samples", 0),
                    "metrics": hybrid_precision_summary
                })
                ...
                for idx, result in enumerate(results):
                    processed_result = {
                        ...
                        "hybrid_context_precision": result.get("hybrid_context_precision", 0.0),
                        "avg_hybrid_score": result.get("avg_hybrid_score", 0.0),
                        "entropy_confidence": result.get("confidence_metrics", {}).get("entropy_confidence", 0.0),
                        "mutual_information_confidence": result.get("confidence_metrics", {}).get("mutual_information_confidence", 0.0),
                        "statistical_significance": result.get("confidence_metrics", {}).get("statistical_significance", 0.0),
                        "domain_confidence": result.get("confidence_metrics", {}).get("domain_confidence", 0.0),
                        "dense_weight": result.get("adaptive_weights", {}).get("dense_weight", 0.0),
                        "sparse_weight": result.get("adaptive_weights", {}).get("sparse_weight", 0.0),
    ...
    def save_comprehensive_report(self, output_file: str = "experiment3_comprehensive_analysis.json"):
        """
        保存综合分析报告
        Args:
            output_file: 输出文件名
        ...
        report = {
            "experiment_info": {
                "experiment_type": "experiment3_advanced_hybrid_precision",
    ...
    def _generate_summary_report(self, comprehensive_report: Dict[str, Any]):
        ...
        print("\n" + "="*80)
        print("🎯 实验三（高级Hybrid Precision算法）综合性能报告")
        print("="*80)
...
def main():
    """主函数"""
    print("🚀 开始实验三（高级Hybrid Precision算法）综合分析...")
    print("="*80)

    # 创建分析器
    analyzer = Experiment3ResultsAnalyzer()
    ...
    print("\n✅ 实验三（高级Hybrid Precision算法）综合分析完成！")
    print("📁 详细结果已保存到 experiment3_comprehensive_analysis.json")
