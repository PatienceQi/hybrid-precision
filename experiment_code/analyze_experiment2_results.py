"""
实验二（Simple Hybrid + 标准RAGAS）结果合并与分析脚本
用于提取和合并5个批次的实验二结果，生成综合性能报告
"""
...
class Experiment2ResultsAnalyzer:
    ...
    def load_batch_results(self) -> bool:
        ...
        Returns:
            是否成功加载所有文件
        """
        print("📊 开始加载实验二（Simple Hybrid + 标准RAGAS）批次结果...")

        for batch_id in range(1, 6):
            result_file = self.batch_results_dir / f"hybrid_standard_batch_{batch_id}_200_samples_results.json"
            ...
            try:
                ...
                hybrid_standard_summary = summary.get("hybrid_standard", {})

                self.batch_summaries.append({
                    "batch_id": batch_id,
                    "total_samples": summary.get("total_samples", 0),
                    "metrics": hybrid_standard_summary
                })
    ...
    def save_comprehensive_report(self, output_file: str = "experiment2_comprehensive_analysis.json"):
        """
        保存综合分析报告
        Args:
            output_file: 输出文件名
        ...
        report = {
            "experiment_info": {
                "experiment_type": "experiment2_simple_hybrid_standard_ragas",
    ...
    def _generate_summary_report(self, comprehensive_report: Dict[str, Any]):
        ...
        print("\n" + "="*80)
        print("🎯 实验二（Simple Hybrid + 标准RAGAS）综合性能报告")
        print("="*80)
...
def main():
    """主函数"""
    print("🚀 开始实验二（Simple Hybrid + 标准RAGAS）综合分析...")
    print("="*80)

    # 创建分析器
    analyzer = Experiment2ResultsAnalyzer()
    ...
    print("\n✅ 实验二（Simple Hybrid + 标准RAGAS）综合分析完成！")
    print("📁 详细结果已保存到 experiment2_comprehensive_analysis.json")
